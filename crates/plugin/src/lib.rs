...
///
/// Each file has its own key-value store, so the plugin can only access the data of the current
...
interface kv-store {
    ...
    ///
    /// # Returns
    /// A list of lists of bytes, where each inner list represents a key that matches the
    /// prefix (with the prefix cutted out).
...
///
/// Only the changes of the current file being processed are accessible but any file content
...
interface repository {
    ...
    ///
    /// # Returns
    /// An optional u64 representing the size of the content.
    /// If the content does not exist, returns `None`.
    ...
    ///
    /// # Returns
    /// An optional string representing the hash of the new content.
    /// If the content to cut does not exist, returns `None`.
...
world plugin {
    ...
    ///
    /// This function should use the `render-bytes` and `render-content` functions
    /// to render the file.
    ...
    ///
    /// After unapplying a change, the file should be in the same state as before
    /// applying the change.
    ///
    /// # Arguments
